...
if __name__ == '__main__':
    working_example_totem = '../data/WorkingExampleTOTeM'
    container_logistics = '../data/ContainerLogistics'
    ...
    log = working_example_totem
    # Define output directory
    base_dir = Path('graphviz_output')
    log_dir = os.path.basename(log)
    output_parent_dir = base_dir / log_dir

    event_stream = EventStream(log + '.json')

    # ocpn_buf = OcpnBuffer(50, 50, 50, cps[4], use_mixed_ocdfg_buf=True, coupled_removal=True, pp_buf=pps[4], pp_buf_dfgs=pps[3])
    # ocpn_buf.process_stream(event_stream.stream[0:2])
    # print(ocpn_buf)
    # ocpn_onl = OcpnModel(ocpn_buf)
    ...
    ocpn_offl = OcpnModel(working_example_totem + '.xml')
    ocpn_offl.visualize(
        output_parent_dir / ocpn_offl.get_graphviz_subdir(),
        ocpn_offl.get_graphviz_file_name(),
        ot_to_hex_color
    )

    # visualize_ocpn_pm4py(container_logistics, visualize_pns=True)
    # print(get_ocpn_accuracy(ocpn_offl, ocpn_onl))