...
if __name__ == '__main__':
    ...
    pps = {0: None,
           ...
           6: PPBObjectsPerEvent(prio_order=PrioPolicyOrder.MAX, window_size=100),
           7: PPBObjectsPerObjectType(prio_order=PrioPolicyOrder.MAX),
           8: PPBEventsPerObjectType(prio_order=PrioPolicyOrder.MAX)}
    pp_id = 0
    ...
    base_dir = Path('graphviz_output')
    log_dir = os.path.basename(logs[log_id])
    output_parent_dir = base_dir / log_dir

    event_stream = EventStream(logs[log_id] + '.json', o2o_has_time=False, enrich_o2o=False)
    ...
    totem_buf = TotemBuffer(7, 7, 7, CachePolicy.FIFO, pp_buf=PPBEventsPerObjectType(prio_order=PrioPolicyOrder.MIN), coupled_removal=True)
    totem_buf.process_stream(event_stream.stream, enrich_o2o=False)
    totem_model = TotemModel(totem_buf, tau=tau, verbose=False)  
    # totem_model.visualize(
    #     output_parent_dir / totem_model.get_graphviz_subdir(),
    #     totem_model.get_graphviz_file_name(), 
    #     ot_to_hex_color
    # )

    totem_model_offline = TotemModel(logs[log_id] + '.xml', tau=tau, verbose=False)
    # totem_model_offline.visualize(
    #     output_parent_dir / totem_model_offline.get_graphviz_subdir(),
    #     totem_model_offline.get_graphviz_file_name(),
    #     ot_to_hex_color
    # )
    
    # print(get_totem_accuracy(totem_model_offline, totem_model))
    visualize_totem_overlap(
        totem_model_offline, 
        totem_model, 
        output_parent_dir / totem_model.get_graphviz_subdir(),
        totem_model.get_graphviz_overlap_file_name(totem_model_offline.tau), 
        ot_to_hex_color
    )