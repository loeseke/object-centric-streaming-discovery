...
if __name__ == '__main__':
    logs = ['../data/WorkingExampleTOTeM.json',
            '../data/ContainerLogistics.json',
            '../data/AgeOfEmpires10.json']
    log_id = 1

    cps = {0: CachePolicy.FIFO, 
           1: CachePolicy.LRU, 
           2: CachePolicy.LFU, 
           3: CachePolicy.LFU_DA, 
           4: CachePolicy.RR}
    cp_id = 1

    pps = {0: None,
           1: PPBStridePerObject(prio_order=PrioPolicyOrder.MAX), 
           2: PPBStridePerObjectType(prio_order=PrioPolicyOrder.MAX, window_size=10, max_obj_per_ot=10, freeze_or_max_idle=pd.Timedelta(1, unit='day')),
           3: PPBLifespanPerObject(prio_order=PrioPolicyOrder.MIN), 
           4: PPBLifespanPerObjectType(prio_order=PrioPolicyOrder.MIN, max_obj_per_ot=10, freeze_or_max_idle=pd.Timedelta(3, unit='day')),
           5: PPBCustom(ot_list=['Handling Unit', 'Container', 'Customer Order', 'Vehicle', 'Truck', 'Forklift', 'Transport Document'], prio_order=PrioPolicyOrder.MAX),
           6: PPBObjectsPerEvent(prio_order=PrioPolicyOrder.MIN, window_size=100),
           7: PPBObjectsPerObjectType(prio_order=PrioPolicyOrder.MAX),
           8: PPBEventsPerObjectType(prio_order=PrioPolicyOrder.MIN)}
    pp_id = 7

    # Define name of output directory
    log_dir = Path('monitor_output',
                    os.path.basename(os.path.splitext(logs[log_id])[0]))
    event_stream = EventStream(logs[log_id])

    # model_buf = OcpnBuffer(10, 10, 10, cps[cp_id], use_mixed_ocdfg_buf=True, max_counter=1000, pp_buf=pps[pp_id], coupled_removal=False, runtime_monitor=RuntimeMonitor())
    # model_buf.process_stream(event_stream.stream)
    # model_buf.create_monitor_dataframes()
    # create_all_plots_runtime_monitor(model_buf.rt_mon, log_dir / get_model_output_path(model_buf))

    # model_buf = OcdfgBuffer(100, 100, CachePolicy.LFU_DA, max_counter=100000, pp_buf=PPBStridePerObjectType(prio_order=PrioPolicyOrder.MAX, freeze_or_max_idle=pd.Timedelta(days=1)), coupled_removal=True, runtime_monitor=RuntimeMonitor())
    # model_buf.process_stream(event_stream.stream)
    # model_buf.create_monitor_dataframes()

    model_buf = TotemBuffer(125, 125, 125, CachePolicy.FIFO, max_counter=100000, pp_buf=PPBEventsPerObjectType(prio_order=PrioPolicyOrder.MIN), coupled_removal=False, cache_monitor=CacheMonitor())
    model_buf.process_stream(event_stream.stream)
    model_buf.create_monitor_dataframes()

    print(log_dir / get_model_output_path(model_buf))
    create_all_plots_cache_monitor(model_buf.c_mon, log_dir / get_model_output_path(model_buf))