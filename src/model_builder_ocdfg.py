...
if __name__ == '__main__':
    ...
    ocdfg_buf = OcdfgBuffer(
        ...
        125, 
        CachePolicy.FIFO, 
        pp_buf=PPBEventsPerObjectType(prio_order=PrioPolicyOrder.MAX), 
        coupled_removal=True
    )
    ocdfg_buf.process_stream(event_stream.stream)

    ocdfg_model = OcdfgModel(
        ocdfg_buf, 
        prune_node_frac=0,
        prune_arc_frac=0,
        verbose=False
    )
    # ocdfg_model.visualize(
    #     output_parent_dir / ocdfg_model.get_graphviz_subdir(), 
    #     ocdfg_model.get_graphviz_file_name(),
    #     ot_to_hex_color,
    #     visualize_dfgs=True
    # )

    ocdfg_model_offline = OcdfgModel(
        logs[log_id] + '.xml', 
        prune_node_frac=0,
        prune_arc_frac=0,
        normalize=False,
    ...
    # ocdfg_model_offline.visualize(
    #     output_parent_dir / ocdfg_model_offline.get_graphviz_subdir(),
    #     ocdfg_model_offline.get_graphviz_file_name(),
    #     ot_to_hex_color
    # )
    
    # print(get_ocdfg_accuracy(ocdfg_model_offline, ocdfg_model))
    visualize_ocdfg_overlap(
        ocdfg_model_offline, 
        ocdfg_model, 
        output_parent_dir / ocdfg_model.get_graphviz_subdir(),
        ocdfg_model.get_graphviz_overlap_file_name(ocdfg_model_offline.prune_node_frac, ocdfg_model_offline.prune_arc_frac), 
        ot_to_hex_color
    )